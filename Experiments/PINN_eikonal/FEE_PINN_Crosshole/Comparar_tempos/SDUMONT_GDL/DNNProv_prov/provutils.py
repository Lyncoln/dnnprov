...
class EikonalSolver2D(object):
    def __init__(self, layers_vel, layers_tt,
        ...
        self.dataflow_tag = "PINN_eikonal_tempos"
        self.exec_tag = self.dataflow_tag + datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def train(self, Nx, Ny):
        ...
        #t2 = Task(2, self.dataflow_tag, self.exec_tag, "Adaptation", dependency=t1)
        #t2.begin()   
        ...
        while it < self.adam_its:
            ...
            if it % 100 == 0:
                ...
                tf1_output = DataSet("oTrainingModel", [Element([it, elapsed, loss_value, Lb, Lr, Ld,r2])])
                t1.add_dataset(tf1_output)
