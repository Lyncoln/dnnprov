...
class EikonalSolver2D(object):
    def __init__(self, layers_vel, layers_tt,
        ...
        self.df = pd.DataFrame(columns=["iteracao","tempo","valor_loss","lb","ld","lr","learning_rate","r2"])
    ...
    def train(self, Nx, Ny):
        ...
        #t1.save()
        #t2.save()
        aux = f'execucoes/{datetime.today().strftime("%Y%m%d_%H%M%S")}'
        if not os.path.exists(aux):
        	os.mkdir(aux)

        self.df.to_csv(aux+"/"+self.path+".csv")
